...
function getMembershipDate(teamUser) {
  // Try common field names coming from APIs/ORMs
  // (TeamUser row can have its own createdAt/updatedAt)
  return (
    formatISODate(teamUser?.joinedAt) ||
    formatISODate(teamUser?.assignedAt) ||
    formatISODate(teamUser?.createdAt) ||
    formatISODate(teamUser?.date) ||
    null
  );
}
    ...
    const teams = response.data.map((team) => ({
      ...
      manager: {
        ...
        avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(
          team.Users.firstname + " " + team.Users.lastname
        )}&background=random`,
      },
      members: team.TeamUser.map((tu) => ({
        ...
        avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(
          tu.Users.firstname + " " + tu.Users.lastname
        )}&background=random`,
        // ✅ Completed TODO: use membership date if available in TeamUser relation row
        date: getMembershipDate(tu),
